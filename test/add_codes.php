...
$sql = "SELECT t.id,t.user_id FROM transactions t,users u WHERE t.user_id = u.id AND u.l_referral_count = 1 AND t.type = 'refer_bonus' AND DATE(t.datetime) = '2023-08-08' GROUP BY user_id";
$db->sql($sql);
$res = $db->getResult();
$num = $db->numRows($res);
if ($num >= 1) {
    ...
    $response['message'] = "1st refer time Bonus Done";
