    ...
    if ($_FILES["upload_file"]["size"] > 0  && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            if ($count1 != 0) {
                ...
                $sql = "SELECT * FROM `users` WHERE `mobile` = '$emapData[1]'";
                $db->sql($sql);
                $res = $db->getResult();
                $num = $db->numRows($res);
                if ($num == 1){
                    $sql = "UPDATE users SET name='$emapData[0]',mobile='$emapData[1]',password='$emapData[2]', 
                    dob='$emapData[3]',email='$emapData[4]',city='$emapData[5]', referred_by='$emapData[6]', 
                    earn='$emapData[7]',withdrawal='$emapData[8]',withdrawal_status='$emapData[9]',total_referrals='$emapData[10]',today_codes='$emapData[11]',
                    total_codes='$emapData[12]',balance='$emapData[13]',device_id='$emapData[14]',
                    status='$emapData[15]',refer_code='$emapData[16]',refer_bonus_sent='$emapData[17]',
                    code_generate='$emapData[18]',code_generate_time='$emapData[19]',last_updated='$emapData[20]',
                    joined_date='$emapData[21]' WHERE mobile='$emapData[1]'";
                    $db->sql($sql);
                }
                else{
