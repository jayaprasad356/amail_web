     ...
     !empty($branch_id)) {
        ...
        if($status == 1 && !empty($referred_by) && $refer_bonus_sent != 1){
            ...
            if ($num == 1){
                ...
                $sql_query = "UPDATE users SET `total_referrals` = total_referrals + 1,`worked_days` = worked_days + 1,`earn` = earn + $referral_bonus,`balance` = balance + $referral_bonus,`salary_advance_balance`=salary_advance_balance +$refer_sa_balance,`sa_refer_count`=sa_refer_count + 1 WHERE id =  $user_id";
                $db->sql($sql_query);
